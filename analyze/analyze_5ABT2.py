    ...
    for i in range(len(section_line) - 1):
        if is_prob:  # 確率である時、maxを1、minを0にする
            ...
            if base_w:
                ...
                plt.vlines(section_line[i], min(np.min(array), base_w), max(np.max(array), base_w), "blue", linestyles='dashed')
            else:
...
def estimate_learning_rate_and_beta_importancesampling(mice, tasks, model, sample_size):
    ...
    for mouse_id in mice:
        ...
        print(f"[{mouse_id}] estimating learning rates with importance sampling")
        ...
        agent = Agent(average_action_array, act_and_rew, section_line, num_first_to_remove)
        agent.set_unconstrained_sim_info(tasks, section_line, get_tasks_prob_dict())
        keys = [k for k in model['pbounds'].keys()]
        ...
        def eval_like(agent, values):
            minus_ll = agent.only_values_sim_f(values)
        ...
        def save_sim(estimated_params, dirname, is_save=False, is_show=False):
            ...
            ll_array = calculate_ll(act_and_rew, prob_array, num_first_to_remove, section_line)
            error_array = calculate_error(average_action_array, prob_array, num_first_to_remove)
            my_graph_plot(q_value_array, section_line, "q_value_graph", "total timestep", "q value", dirname=dirname, is_save=is_save, is_show=is_show)
            my_graph_plot(prob_array, section_line, "prob_graph", "total timestep", "prob", dirname=dirname, is_save=is_save, is_show=is_show, is_prob=True)
            my_graph_plot(ll_array, section_line, "ll_graph", "total timestep", "ll", dirname=dirname, is_save=is_save, is_show=is_show)
            my_graph_plot(error_array, section_line, "error_graph", "total timestep", "error", dirname=dirname, is_save=is_save, is_show=is_show)
            return np.sum(ll_array), np.sum(error_array)

        def save_unconstrained_sim(estimated_params, dirname, is_save=False, is_show=False, num_sim=1000):
            os.makedirs(dirname, exist_ok=True)
            q_value_array, prob_array = agent.mean_unconstrained_sim(estimated_params, num_sim)
            ll_array = calculate_ll(act_and_rew, prob_array, num_first_to_remove, section_line)
            error_array = calculate_error(average_action_array, prob_array, num_first_to_remove)
            my_graph_plot(q_value_array, section_line, "q_value_graph", "total timestep", "q value", dirname=dirname, is_save=is_save, is_show=is_show)
            my_graph_plot(prob_array, section_line, "prob_graph", "total timestep", "prob", dirname=dirname, is_save=is_save, is_show=is_show, is_prob=True)
        ...
        if is_unco_check:
            unconstrained_sim_ll, unconstrained_sim_error = save_unconstrained_sim(estimated_params, dirname + "uncostrained_sim/", is_save=is_save, is_show=False, num_sim=NUM_SIM)

            _, unco_prob_arrays = agent.unconstrained_sim_return_arrays(estimated_params, num_sim=NUM_SIM)
            unco_ll_arrays = parallel_calculate_ll(act_and_rew, unco_prob_arrays, num_first_to_remove)
            unco_ll_array = np.mean(unco_ll_arrays, axis=0)
            unco_ll = np.sum(unco_ll_array)
            print("unconstrained_each_ll: ", unco_ll)
            print("unconstrained_mean_ll: ", unconstrained_sim_ll)
        ...
        estimated_params_ser['mouse_id'] = mouse_id
        estimated_params_ser['ll'] = ll
        estimated_params_ser['model'] = model["model_name"]
        estimated_params_ser["error"] = error
        if is_unco_check:
            estimated_params_ser["unconstrained_mean_ll"] = unconstrained_sim_ll
            estimated_params_ser["unconstrained_sim_error"] = unconstrained_sim_error
            estimated_params_ser["unconstrained_each_ll"] = unco_ll
            estimated_params_ser["target"] = ll + unco_ll
        # ここまでOptimisationからのほぼコピペ

        if 'df_estimation_is' in locals():
            df_estimation_is = df_estimation_is.append(estimated_params_ser, ignore_index=True)
        else:
...
def calc_task_duration(mice):
    df_summary = pd.DataFrame(columns=['mouse_id', 'task', 'duration', 'reward', 'failure', 'timeover'])
    ...
    for mouse_id in mice:
        ...
        for task in tasks_in_log:
            ...
            if verbose_level > 0:
                print(
                    f"[calc_task_duration] mouse_id={mouse_id}, task={task: <13}: duration={duration}")

            ent_row = pd.Series([mouse_id, task, success, failure, timeover, duration], index=df_summary.columns)
            df_summary = df_summary.append(ent_row, ignore_index=True)
