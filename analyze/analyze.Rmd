...
```{r malcov eval=FALSE}
func_markov <- function(dem){
  if(dem > length(data)){
    dem <- length(data)
  }
  d <- data[(length(data)-dem):length(data)]
  retval <- c()
  for (number in order(unique(data$hole_no))){
    retval <- append(retval,(length(d[[d$hole_no==number]]+1))/(length(d)+2))
  }  
  return(retval)
}
```
...
alpha <- c(0.5,0)
kappa <- c(0.5,0)
func_standard_RLalgo <- function(datas,param){
  # 定義
  ids <- datas %>%
  ...
  for(t in ids){
    ...
    for(hole in holes){
      ...
      Q[which(holes==hole),which(ids==t)+1] <- learning_late*Q[which(holes==hole),which(ids==t)] + disgust
    }
  }
  # 全holeの選択確率を更新
