...
# behavior_data <- read_csv('../RaspSkinnerBox/log/cage1_no2.txt', col_names=header, col_types='Tciicic')
behavior_data <- read_csv('../RaspSkinnerBox/log/no013_action.csv', col_names=header, col_types='Tciicic')
...
dt$is_correct = ifelse(dt$event_type=='reward', 1, NA)
dt$is_incorrect = ifelse(dt$event_type=='failure', 1, NA)
dt$is_omission = ifelse(dt$event_type=='time over', 1, NA)
...
## バーストごとhole
### TODO
- x軸timestamp,バースト情報の乗せ方
- 一定バースト期間での区切り方
```{r burst_nosepoke}
num_burst <- ggplot(dt,aes(y=burst_group))
odd <- seq(1, max(dt$burst_group), by = 2)
odd_flag <- odd
odd_flag[odd_flag>=1]<- 1

hole_burst_graph <- ggplot(dt) +
  # geom_rect(data = dt, aes(xmin = xmin, xmax = xmax, ymin = -Inf, ymax = Inf,fill=odd_flag),alpha=0.3)+  
  geom_point(aes(y=(is_hole1)*1,x=timestamps,colour=event_type))+
  geom_point(aes(y=(is_hole3)*2,x=timestamps,colour=event_type))+
  geom_point(aes(y=(is_hole5)*3,x=timestamps,colour=event_type))+
  geom_point(aes(y=(is_hole7)*4,x=timestamps,colour=event_type))+
  geom_point(aes(y=(is_hole9)*5,x=timestamps,colour=event_type))+    
  geom_point(aes(y=is_omission*0,x=timestamps,colour=event_type))+
  guides(colour=FALSE)+
  xlab("time/sessions")+
  ylab("hole dots")+
  # theme(axis.text.x = element_blank(),
...
## Correct 時の同選択肢選択確率1-4
## Incorrect 時の異選択肢選択確率1-4
## Correct,Incorrect 時の Omittion 確率1-4

```{r calc_prob}
after_c_all <- nrow(filter(dt,event_type=="reward"))
after_f_all <- nrow(filter(dt,event_type=="failure"))
after_o_all <- nrow(filter(dt,event_type=="time over"))
# TODO データフレームとして作成
prob <- data.frame()
prob$c_same <- c(0,0,0,0)
prob$f_diff <- c(0,0,0,0)
prob$c_omit <- c(0,0,0,0)
prob$f_omit <- c(0,0,0,0)
prob$c_NotMax <- c(0,0,0,0)
prob$f_NotMax <- c(0,0,0,0)
prob$o_NotMax <- c(0,0,0,0)
for(i in 1:nrow(df)){
  if (dt$event_type[i]=="reward") {
    for (j in 1:4) {
      if (dt$hole_no[i] == dt$hole_no[i+j]) {
        prob$c_same[j] %+=% 1
      }else if (dt$is_omission[i+j]) {
        prob$c_omit[j] %+=% 1
      }
    }    
  }else if (dt$event_type[i]=="failure") {
    for (j in 1:4) {
      if(dt$is_omission[i+j]){
        prob$f_omit %+=% 1
      }else if (dt$hole_no[i] != dt$hole_no[i+j]) {
        prob$f_diff %+=% 1
      }
    }
  }
}
prob$c_same %/=% after_c_all
prob$f_diff %/=% after_f_all
prob$c_omit %/=% after_c_all
prob$f_omit %/=% after_f_all
